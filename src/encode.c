...
Status read_and_validate_encode_args(char *argv[], EncodeInfo *encInfo)
{
    ...
    //validate input secret file extn .txt
    ...
    //validate input stego Image extn .bmp
    if(argv[4] != NULL)
    {
...
Status encode_data_to_image(char *data, int size, EncodeInfo *encInfo)
{
    ...
    {
        //read 8 byte from sourse file
        fread(encInfo->image_data,8,1,encInfo->fptr_src_image);
        
        encode_byte_to_lsb(data[i], encInfo->image_data);

        //write encode data to output
...
Status copy_remaining_img_data(EncodeInfo *encInfo)
{
    ...
    fread(buffer,size,1,encInfo->fptr_src_image);
    fwrite(buffer,size,1,encInfo->fptr_stego_image);
...
Status do_encoding(EncodeInfo *encInfo)
{
    ...
    {
        printf("Error: Secret data cannot be fit in given image");
        return e_failure;
    }
    return e_success;
}